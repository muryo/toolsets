...
def process_line(line):
	...
	l = line.split(",")
	if l[1] == "Profile": # if it is the table header(freqs' name)
		current_header = []
		i = 3
		while i < len(l):
			if l[i] != "":
				...
				svc_table[l[i]] = {}
			i += 4
	if l[1].isdigit() and int(l[1]) in range(0,16): # it is the table content(vmin value)
		for i in range(0, len(current_header)):
			svc_table[current_header[i]][l[1]] = l[3 + i*4 + 1]

def adjust_volt_table(): # remove some voltage when len(volt_set) exceed 16
	all_volt_set[0].remove(1212500)
	all_volt_set[0].remove(1312500)
	all_volt_set[14].remove(1212500)
	all_volt_set[14].remove(1312500)
	all_volt_set[15].remove(1212500)
	all_volt_set[15].remove(1312500)
	return
...
def gen_volt_map_table():
	...
	for profile in range(0, 16):
		for pp in all_pp_list:
			print str(profile) + pp + svc_table[pp][str(profile)]
...
#	for profile in range(0, 16):
#		print "profile: %d len: %d" % (profile, len(all_volt_set[profile]))
#		print all_volt_set[profile]
	...
	for profile in range(0, 16):
		...
		print "\t",
		while i < 16:
#			print "\t" + str(all_volt_set[profile][i]) + ", \t" + str(all_volt_set[profile][i+1]) + ", \t" + str(all_volt_set[profile][i+2]) + ", \t" + str(all_volt_set[profile][i+3]) + ","
